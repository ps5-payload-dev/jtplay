...
Rml::Input::KeyIdentifier RmlSDL::ConvertControllerButton(int sdlbtn) {
  ...
  // Triangle cycles the audio track during playback; T so a keyboard can
  // reach it. (Circle already covers back/stop, so Escape isn't lost.)
  case SDL_CONTROLLER_BUTTON_Y:             return Rml::Input::KI_T;
