...
/**
 * This symbol is not inluded with Ubutnu 26.04, see:
 * https://github.com/sahlberg/libsmb2/issues/473
 **/
int __attribute__((weak))
smb2_get_nterror(struct smb2_context *smb2) {
  return -1;
}
...
smb_response_perror(struct MHD_Connection *conn, const char* s) {
  ...
  if((resp=MHD_create_response_from_buffer(strlen(buf), buf,
    ...
    MHD_add_response_header(resp, MHD_HTTP_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN, "*");
    ret = MHD_queue_response(conn, http_error, resp);
    MHD_destroy_response(resp);
  }
...
smb_request_path(struct MHD_Connection *conn, const char* user,
  ...
  smb2_set_authentication(smb2, 1); // SMB2_SEC_NTLMSSP
  ...
  if(resp) {
    MHD_add_response_header(resp, MHD_HTTP_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN, "*");
    if(range) {
      ret = MHD_queue_response(conn, MHD_HTTP_PARTIAL_CONTENT, resp);
    } else {
...
smb_request_shares_cb(struct smb2_context *smb2, int status, void *data,
  ...
  if((resp=MHD_create_response_from_buffer(size, buf,
    ...
    MHD_add_response_header(resp, MHD_HTTP_HEADER_CONTENT_TYPE,
                            "application/json");
    MHD_add_response_header(resp, MHD_HTTP_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN, "*");
    args->result = MHD_queue_response(args->conn, MHD_HTTP_OK, resp);
    MHD_destroy_response(resp);
  } else {
    args->result = MHD_NO;
  }
